...
**SNMPv3 — utilisateur**
```
snmp<commande> -v3 -u <user> -l <niveau> -a <auth_algo> -A <auth_pass> -x <priv_algo> -X <priv_pass> <cible> <OID>
```
...
| `-c` | `public`, `supervision`... | Communauté (v1/v2c uniquement) |
| cible | IP ou hostname | Agent à interroger |
| OID | numérique ou symbolique | Objet à lire/modifier |

### Paramètres SNMPv3 uniquement

| Paramètre | Valeurs | Description |
|---|---|---|
| `-u` | `admin_sec`... | Nom d'utilisateur USM |
| `-l` | `noAuthNoPriv`, `authNoPriv`, `authPriv` | Niveau de sécurité |
| `-a` | `SHA`, `MD5` | Algorithme d'authentification |
| `-A` | mot de passe | Mot de passe d'authentification (≥8 car.) |
| `-x` | `AES`, `DES` | Algorithme de chiffrement |
| `-X` | mot de passe | Mot de passe de chiffrement (≥8 car.) |
...
## SNMPv3 — Commandes avec authentification et chiffrement
...
# snmpwalk en authPriv
snmpwalk -v3 -u admin_sec -l authPriv \
  -a SHA -A PassAuth123 \
  -x AES -X PassPriv123 \
  192.168.56.20 system

# snmpget en authNoPriv (auth sans chiffrement)
snmpget -v3 -u admin_sec -l authNoPriv \
  -a SHA -A PassAuth123 \
  192.168.56.20 SNMPv2-MIB::sysName.0

# snmpget en noAuthNoPriv (aucune sécurité)
snmpget -v3 -u admin_sec -l noAuthNoPriv \
  192.168.56.20 SNMPv2-MIB::sysName.0
```
...
|---|---|---|---|
| `noAuthNoPriv` | Non | Non | `-l noAuthNoPriv` |
| `authNoPriv` | Oui | Non | `-l authNoPriv -a SHA -A <pass>` |
| `authPriv` | Oui | Oui | `-l authPriv -a SHA -A <pass> -x AES -X <pass>` |

---

## SNMPv3 — Configuration snmpd.conf

```
# Autoriser un utilisateur SNMPv3 en lecture (niveau minimum authPriv)
rouser admin_sec authPriv

# Autoriser un utilisateur SNMPv3 en lecture (tous niveaux)
rouser admin_sec

# Autoriser un utilisateur SNMPv3 en lecture/écriture
rwuser admin_sec authPriv

# Envoyer les traps en SNMPv3
trapsess -v3 -u admin_sec -l authPriv -a SHA -A PassAuth123 -x AES -X PassPriv123 192.168.56.10
```

### Créer un utilisateur SNMPv3 (agent)

```bash
# snmpd doit être arrêté
sudo systemctl stop snmpd

# Créer l'utilisateur (nécessite libsnmp-dev)
...
| `net-snmp-config: command not found` | `libsnmp-dev` non installé | `apt install libsnmp-dev` |
| `Unknown user name` SNMPv3 | `createUser` non exécuté (fichier existait déjà) | Fichier marqueur `.admin_sec_created` |
| `createUser` ignoré au redémarrage | snmpd tourne pendant l'écriture | Stop snmpd → createUser → Start snmpd |